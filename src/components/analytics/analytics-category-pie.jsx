...
export default function AnalyticsCategoryPie({ filters, transactions = [] }) {
    ...
    <Card className="rounded-2xl border border-slate-200/70 bg-white/80 backdrop-blur shadow-sm dark:border-slate-800 dark:bg-slate-950/40 overflow-hidden">
      <CardHeader className="border-b border-slate-200/70 bg-white/60 dark:border-slate-800 dark:bg-slate-950/20">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-0.5">Distribution by category</p>
        </div>
      </CardHeader>

      <CardContent className="h-[340px] p-4 min-w-0">
        {data.length === 0 ? (
          <div className="h-full w-full min-w-0">No expense data yet.</div>
        ) : (
